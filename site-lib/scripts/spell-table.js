...
(function () {
  ...
  function initSpellTableFilter() {
    ...
    // Показывает только выбранный span, остальные скрывает; при сбросе показывает все
    function setSpecialSpanVisibility(row, colIndex, selectedValue) {
      ...
      // Если точного совпадения не нашлось — не оставляем ячейку пустой
      if (!hasMatch) {
        spans.forEach(span => {
          span.style.display = "";
        });
    ...
    function apply() {
      ...
      rows.forEach(row => {
        ...
        // Показываем только выбранный span в "Особое" и "Сборник"
