...
(function() {
   ...
   function initSpellTableFilter() {
      ...
      // НОВОЕ: скрываем/показываем span в ячейке по выбранному значению фильтра
      function applySpanVisibility(row, colIndex, filterValue) {
         const cell = row.querySelectorAll("td")[colIndex];
         if (!cell) return;
         cell.querySelectorAll("span").forEach((span) => {
            span.style.display = !filterValue || span.textContent.trim() === filterValue ? "" : "none";
         });
      }
      ...
      function getUnique(colIndex, useSpecialSpans = false, splitter = null) {
         ...
         rows.forEach((row) => {
            if (useSpecialSpans) {
               getSpecialSpanValues(row, colIndex).forEach((v) => {
                  if (v && v !== "-") set.add(v);
               });
            } else if (splitter) {
               splitter(cellText(row, colIndex)).forEach((v) => {
                  if (v && v !== "-") set.add(v);
               });
            } else {
      ...
      function updateSortIndicators() {
         ...
         thead.querySelectorAll("th").forEach((th, i) => {
            ...
            icon.style.cssText =
               "margin-left:4px;font-size:11px;opacity:.5;user-select:none;";
            if (i === sortCol) {
               icon.textContent =
                  sortDir === 1 ? " ▲" : sortDir === -1 ? " ▼" : " ⇅";
...
`;
      ...
      const selConc = makeSelect(
         "— Концентрация —",
         getUnique(COL_CONCENTRATION),
      );
      ...
      ["В", "С", "М"].forEach((c) => {
         ...
         label.style.cssText =
            inputCSS +
            "display:flex;gap:4px;align-items:center;cursor:pointer;padding:5px 8px;";
...
`;
      ...
      counter.style.cssText =
         "color:var(--text-muted,#888);font-size:12px;white-space:nowrap;";
      ...
      function apply() {
         ...
         rows.forEach((row) => {
            ...
            const ritualFilterOk = !ritual ||
               (ritual === "(Не Ритуал)" ? !hasRitual : rowRitual.includes(ritual));
            const ok =
               (!q || name.includes(q)) &&
            ...
            if (ok) count++;
            // НОВОЕ: управляем видимостью span в ячейках Особое и Сборник
            applySpanVisibility(row, COL_SPECIAL, special);
            applySpanVisibility(row, COL_SOURCE, source);
         });
      ...
      reset.addEventListener("click", () => {
         ...
         // НОВОЕ: восстанавливаем все span перед сбросом
         rows.forEach((row) => {
            applySpanVisibility(row, COL_SPECIAL, "");
            applySpanVisibility(row, COL_SOURCE, "");
         });
         sortCol = -1;
         sortDir = 0;
