...
(function () {
  ...
  function initSpellTableFilter() {
    ...
    const timeOrder = {
      "Действие": 1, "Бонусное Действие": 2, "Реакция": 3,
      "Минуты": 4, "Часы": 5, "Особое": 6
    };
    ...
    function setSpecialSpanVisibility(row, colIndex, selectedValue) {
      ...
      let hasMatch = false;
      spans.forEach(span => {
        const match = normalize(span.textContent) === selected;
    ...
    function getUnique(colIndex, useSpecialSpans = false, splitter = null) {
      ...
      rows.forEach(row => {
        if (useSpecialSpans) {
          ...
          spanValues.forEach(v => { if (v && v !== "-") set.add(v); });
        } else if (splitter) {
          const val = cellText(row, colIndex);
          const values = splitter(val);
          values.forEach(v => { if (v && v !== "-") set.add(v); });
        } else {
          const val = cellText(row, colIndex);
    ...
    // === КНОПКИ СКРЫТИЯ СТОЛБЦОВ ===
    function toggleColumn(colIndex) {
      if (hiddenCols.has(colIndex)) hiddenCols.delete(colIndex);
    ...
    if (thead) {
      thead.querySelectorAll("th").forEach((th, i) => {
        ...
        // Кнопка скрытия столбца
        const hideBtn = document.createElement("span");
        hideBtn.textContent = "✕";
        hideBtn.style.cssText = "margin-left:6px;font-size:11px;opacity:0.5;cursor:pointer;";
    ...
    bar.id = FILTER_ID;
    bar.style.cssText = `display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;align-items:center;`;
    ...
    reset.style.cssText = `
      padding: 6px 10px; border: 1px solid var(--background-modifier-border, #444);
    ...
    // === Переключатель "Скрыть без Особое" ===
    const specialToggle = document.createElement("label");
    specialToggle.style.cssText = `display:flex;align-items:center;gap:6px;${inputCSS}cursor:pointer;`;
    const specialCheckbox = document.createElement("input");
    specialCheckbox.type = "checkbox";
    specialToggle.append(specialCheckbox, document.createTextNode("Скрыть без «Особое»"));

    bar.append(
      search, selLevel, selSchool, selTime, compBox, selConc,
      selRitual, selSpecial, selSource, specialToggle, reset, counter
    );
    ...
    function apply() {
      ...
      rows.forEach(row => {
        ...
        setSpecialSpanVisibility(row, COL_SPECIAL, special);
        setSpecialSpanVisibility(row, COL_SOURCE, source);
    ...
    reset.addEventListener("click", () => {
      search.value = "";
      selLevel.value = selSchool.value = selTime.value = selConc.value = "";
      selRitual.value = selSpecial.value = selSource.value = "";
    ...
    [search, selLevel, selSchool, selTime, selConc, selRitual, selSpecial, selSource, specialCheckbox]
