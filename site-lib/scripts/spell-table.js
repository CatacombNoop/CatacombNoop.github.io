  ...
  function initSpellTableFilter() {
    ...
    function getUnique(colIndex, useSpecialSpans = false, splitter = null) {
      ...
      rows.forEach(row => {
        ...
        if (useSpecialSpans) {
          ...
          getSpecialSpanValues(row, colIndex).forEach(v => { if (v && v !== "-") set.add(v); });

        } else if (splitter) {

          splitter(cellText(row, colIndex)).forEach(v => { if (v && v !== "-") set.add(v); });
    ...
    if (thead) {
      ...
      thead.querySelectorAll("th").forEach((th, i) => {
        ...
        th.addEventListener("mouseenter", () => { th.style.background = "var(--background-modifier-hover, rgba(255,255,255,0.05))"; });

        th.addEventListener("mouseleave", () => { th.style.background = ""; });

        th.addEventListener("click", e => { if (e.target.tagName === "A") return; sortTable(i); });
